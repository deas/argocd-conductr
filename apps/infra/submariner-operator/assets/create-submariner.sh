...
export SUBMARINER_BROKER_CA=$(kubectl -n ${BROKER_NS} get secrets -o json | jq -r -c '[.items[] | select(.metadata.annotations."kubernetes.io/service-account.name"=="submariner-k8s-broker-client") | select(.data."ca.crt" != null)] | .[0].data."ca.crt"' | base64 --decode)
...
cat "${TEMPLATE}" | envsubst | kubectl -n "${SUBMARINER_NS}" apply -f -

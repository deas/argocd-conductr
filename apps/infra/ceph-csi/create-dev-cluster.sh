...
CLUSTER_FILES="common.yaml operator-test.yaml cluster-test.yaml cluster-on-pvc-minikube.yaml dashboard-external-http.yaml toolbox.yaml"
...
export ROOK_EXAMPLES_DIR="${ROOK_EXAMPLES_DIR:="$SCRIPT_ROOT"/manifests/dev-cluster}" ## Path to Rook examples directory (i.e github.com/rook/rook/deploy/examples)
...
setup_minikube_env() {
  ...
  $MINIKUBE delete
  # Need --cpus=3 when using cephfs-csi driver
  $MINIKUBE start --network=default --container-runtime=containerd --disk-size="$MINIKUBE_DISK_SIZE" --extra-disks="$MINIKUBE_EXTRA_DISKS" --driver "$minikube_driver" -n "$MINIKUBE_NODES" $ROOK_MINIKUBE_EXTRA_ARGS
...
create_rook_cluster() {
  ...
  $KUBECTL apply -f crds.yaml -f common.yaml -f operator-test.yaml

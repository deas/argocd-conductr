...
set-gitops-rev: ## Set gitops control plane REV - defaults to current branch. Machine-owned stage/rendered branches are untouched
	if [ -n "$(REV)" ] ; then rev=$(REV); else rev=$$(git rev-parse --abbrev-ref HEAD); fi ; \
	old_rev=$$(grep targetRevision: envs/kind-helm/app-root.yaml | sed -e s,'.*targetRevision: ',,g) ; \
	find envs -iname "*.yaml" | while read f ; do sed -i "s,evision: $${old_rev}$$,evision: $${rev},g" "$${f}"; done
